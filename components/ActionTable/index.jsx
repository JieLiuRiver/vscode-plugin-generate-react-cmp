...
const getTableData = ({
  ...
  return fetch(`https://randomuser.me/api?results=${pageSize}&${query}`).then(res => res.json()).then(res => ({
    total: 55,
...
const filterColumns = columnList => {
  ...
  return newColumnList.filter(columnItem => {
    ...
    if (columnItem.children) {
      ...
      return <Table.ColumnGroup key={getColumnKey(columnItem)} {...groupProps}>
...
const AppList = () => {
  ...
  const [zebraStatus, changeZebra] = useState(false); // 切换全屏

  const [, {
    toggleFull
  }] = useFullscreen(document.getElementById('table-container'), {
    onFull: () => {
  ...
  const {
    paginationProps,
    tableProps
  } = useFusionTable(getTableData, {}); // 切换当前 columns

  const [columns, onColumnChange] = useState(defaultColumns);
  return <Card free className={styles.container} id="table-container">
      <Card.Content>
